...
int KeypadMalfunction();
int KeypadPasswordGenerator();
int KeypadFight();
int KeypadDodge(int Digit, int move);
						...
						case 16:
			{
					while(choice != 99)
					{
					    ...
					    int DigitOne, DigitTwo, DigitThree, DigitFour;
					        ...
					        puts("As you start to run to the door to avoid drowning");
					        puts("You run past door 13...");
					        puts("......14..");
					        puts("..15......");
					        puts("Then you get to door 61\n");
					        puts("???????????\n");
					        
							fakeNumber = KeypadMalfunction();
							printf("The keypad flashes %d \n", fakeNumber);
							
							puts("Then it beeps once and flashes four X's");
							puts("Out of frustration you punch the keypad");
							DigitOne = KeypadPasswordGenerator();
							DigitTwo = KeypadPasswordGenerator();
							DigitThree = KeypadPasswordGenerator();
							DigitFour = KeypadPasswordGenerator();
							
							printf("It then beeps again and flashes the number %d-%d-%d-%d\n", DigitOne, DigitTwo, DigitThree, DigitFour);
							puts("Then the door opens and you go inside.");
							puts("The moment you step inside three knives hit the wall around you.");
							puts("You look up and there is a 3 armed man with a knife in each hand.");
							puts("He tells you that you have entered the reverse room where everything is backwards.\n");
							
							puts("You must dodge his knives four times if you wish to escape.");
							puts("If not you die.\n");
							
							KeypadFight(DigitOne, DigitTwo, DigitThree, DigitFour);
...
int KeypadMalfunction()
{
    ...
    puts("You figure the door number is just on backwards\n");
    puts("You turn the handle, but the door is locked.");
    puts("So you look around, and notice a keypad next to the door.");
    puts("The keypad only takes the numbers 1 through 4.");
    puts("So reach out and choose a number.");
    
    puts("Which number do you choose?\n");
    scanf("%d", &fakeNumber);
        do
        {
            if(fakeNumber > 0 && fakeNumber < 5)
            {
            puts("........ Nothing happens");
            puts("You decide to try another number");
        ...
        while(fakeNumber < 1 || fakeNumber > 4);
...
int KeypadFight(int DigitOne, int DigitTwo, int DigitThree, int DigitFour)
{
    
    int arrMove[] = {DigitFour, DigitThree, DigitTwo, DigitOne};
    
    int dodge;
    
    puts("THE RULES ARE SIMPLE.... ");
    puts("WHEN THE THREE ARMED MAN THROWS KNIVES AT YOU, DODGE ALL THREE.");
    puts("1 = MOVE LEFT");
    puts("2 = JUMP");
    puts("3 = MOVE RIGHT");
    puts("4 = DUCK\n");
    ...
    for(int i = 0; i < 4; i++)
    {
        puts("The three armed man throws the knives at you");
        puts("Which way do you want to move?");
        puts("Choose 1 - 4");
        scanf("%d", &dodge);
        ...
        if(dodge < 1 || dodge > 4)
        {
            ...
            while(dodge < 1 || dodge > 4);
        }
        
        KeypadDodge(arrMove[i], dodge);
    }
...
int KeypadDodge(int Digit, int move)
{
    if(Digit == move)
    {
        puts("You guessed correctly and dodged the 3 knives");
    }
    else
    {
        puts("You were hit, and you have died");
        exit(0);
